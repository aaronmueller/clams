...
parser.add_argument('--lm_data', type=str, default=None,
...
parser.add_argument('--nhid', type=int, default=800,
...
corpus = datatest.TestCorpus(args.lm_data, args.testfname, args.save_lm_data)#'/home-1/amuelle8@jhu.edu/grammars/lstm_lm.bin')

if args.test:
    torch.no_grad()
    test_lm_sentences, test_lm_data, test_lm_grammatical = corpus.test_lm
else:
...
def get_complexity_apply(o,t,sentid,is_grammatical,tags=False):
    ## Use apply() method
    ...
    for corpuspos,targ in enumerate(t):
        ...
        if args.guess:
            ...
            print(str(word)+' '+str(sentid)+' '+str(corpuspos)+' '+str(len(word))+' '+str(float(surp))+' '+str(float(Hs[corpuspos]))+' '+str(outputguesses)+' '+str(is_grammatical))
        else:
            print(str(word)+' '+str(sentid)+' '+str(corpuspos)+' '+str(len(word))+' '+str(float(surp))+' '+str(float(Hs[corpuspos]))+' '+str(is_grammatical))
...
def test_evaluate(test_lm_sentences, lm_data_source, test_lm_grammatical):
    # Turn on evaluation mode which disables dropout.
    model.eval()
    total_loss = 0.
    ntokens = len(corpus.dictionary)
    if args.words:
        print('word sentid sentpos wlen surp entropy grammatical')#,end='')
        if args.guess:
    ...
    for i in range(len(lm_data_source)):
        ...
        sent_grammatical = test_lm_grammatical[i]
        if args.cuda:
        ...
        if args.words:
            ...
            get_complexity_apply(output_flat,targets,i,sent_grammatical)
        else:
...
def evaluate(lm_data_source):
    ...
    for i in range(0, lm_data_source.size(0) - 1, args.bptt):
...
if not args.test:
    ...
    # Run on test data.
    test_loss = test_evaluate(test_lm_sentences, test_lm_data, test_lm_grammatical)
    print('=' * 89)
